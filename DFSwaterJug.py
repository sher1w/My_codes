...
def dfs_water_jug(cap1, cap2, target1, target2=None):
    visited = set()
    stack = [((0, 0), [])]  # Each element is ((jug1, jug2), path)
    ...
    while stack:
        ...
        next_states = [
            ...
            (0, jug2),     # Empty jug1
            (jug1, 0),     # Empty jug2
            (jug1 - min(jug1, cap2 - jug2), jug2 + min(jug1, cap2 - jug2)),  # Pour jug1 -> jug2
            (jug1 + min(jug2, cap1 - jug1), jug2 - min(jug2, cap1 - jug1))   # Pour jug2 -> jug1
        ]

        for state in next_states:
            if state not in visited:
                stack.append((state, path))  # DFS uses stack (LIFO)
...
if target1 > cap1 or (target2 is not None and target2 > cap2):
    ...
    path = dfs_water_jug(cap1, cap2, target1, target2)
    if path:
        print("\n✅ Solution found (DFS):")
        print_solution(path)
    else:
